...
class Welcome extends Phaser.Scene {
    ...
    constructor() {
        super({
            key: 'Welcome',
            backgroundColor: '#2222', // Configuração da cor de fundo da cena
        });
    }
    ...
    preload() {
        ...
        this.load.image("play", "img/play.png"); // Carregando a imagem do botão "play"
    }
    ...
    create() {
        ...
        var text = { height: 20, padding: 15, content: "Bem-vindo" }
        ...
        this.playBt = this.add.image(this.game.config.width / 2 - 50, this.game.config.height / 4 * 3, 'play')
            .setScale(.3).setOrigin(0, 0).setInteractive().setVisible(false);
    ...
    updateName(inputNameElement) {
        ...
        if (name.value != "") {
            this.message.setText("Eai, " + name.value);
